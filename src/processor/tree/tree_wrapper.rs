...
use state_reconstruct_fetcher::{
    constants::{
        ethereum::{DA_URL, HTTP_URL, VERIFY_HELPER_ADDR},
        l3::{CONTRACT_ADDRESSES, PROOF_SIZE, SLOT_ID_START},
        storage::{INITAL_STATE_PATH, INNER_DB_NAME},
    },
    types::CommitBlock,
};
...
impl TreeWrapper {
    ...
    pub async fn new(
        ...
        if reconstruct {
            ...
            reconstruct_genesis_state_enc2(&mut tree, &mut guard, INITAL_STATE_PATH)?;
        }
    ...
    pub async fn insert_block(&mut self, block: &CommitBlock) -> Result<RootHash> {
        ...
        for (key, value) in &block.initial_storage_changes {
            ...
            changed_keys.insert(key.clone(), true);
            self.inner_db
                .lock()
                .await
                .add_key(key)
        ...
        if root_hash_bytes == block.new_state_root {
            tracing::info!("Successfully processed block {}", block.l2_block_number);

            // check L3 contracts
            let version = self.tree.latest_version().unwrap_or_default();
            for contract_address in CONTRACT_ADDRESSES.iter() {
                let num_slots = (PROOF_SIZE + 31) / 32;
                let mut slot_ids = vec![];
                let slot_id_start = U256::from_str_radix(SLOT_ID_START, 16)?;
                for i in 0..num_slots {
                    slot_ids.push(slot_id_start + U256::from(i));
                }
                ...
                for slot_id in slot_ids.iter() {
                    ...
                    let derived_key = derive_final_address_for_params(&contract_address, &slot_id);
                ...
                if !has_proof {
                    tracing::info!(
                        "no new proof found for contract {} by key changes",
                        contract_address
                    );
                    continue;
                }

                let res = self.tree.entries(version, &keys)?;
                let mut concatenated = vec![];

                for entry in res {
                    concatenated.extend_from_slice(&entry.value.as_bytes());
                }

                concatenated.truncate(PROOF_SIZE as usize);
                tracing::debug!(
                    "proof from tree queries: {:?}",
                    hex::encode(concatenated.clone())
                );

                let provider: Provider<Http> = Provider::<Http>::try_from(DA_URL)
                ...
                if !proof_valid {
                    tracing::error!("verify L3 proof failed");
                    return Err(TreeError::BlockMismatch.into());
                }
                tracing::info!("verify L3 proof succeeded");
            }
            Ok(root_hash)
        } else {
            tracing::error!(
                "Unsolved error: Root hash mismatch!\nLocal: {}\nPublished: {}",
...
fn reconstruct_genesis_state_enc2<D: Database>(
    tree: &mut MerkleTree<D>,
    snapshot: &mut ReconstructionDatabase,
    path: &str,
) -> Result<()> {
    ...
    for line in input.lines() {
        let mut separated = line.split(',');
        let derived_key = separated.next().unwrap();
        let value = separated.next().unwrap();

        let key = U256::from_str_radix(cleanup_encoding(derived_key), 10)?;
        let value = U256::from_str_radix(cleanup_encoding(value), 16)?;
    ...
    let mut tree_entries = Vec::with_capacity(block_batched_accesses.len());
    let mut index = 1;
    for (derived_key, value) in block_batched_accesses {
        // let derived_key = derive_final_address_for_params(&address, &key);
        let mut tmp = [0u8; 32];
        value.to_big_endian(&mut tmp);
        ...
        // let key = U256::from_little_endian(&derived_key);
        let value = H256::from(tmp);
        tree_entries.push(TreeEntry::new(derived_key, index, value));
        snapshot
            .add_key(&derived_key)
